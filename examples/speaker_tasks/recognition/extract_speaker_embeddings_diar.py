...
def get_embeddings(speaker_model, manifest_file, batch_size=1, embedding_dir='./', device='cuda'):
    ...
    with open(manifest_file, 'r', encoding='utf-8') as manifest:
        for i, line in enumerate(manifest.readlines()):
            line = line.strip()
            dic = json.loads(line)
            uniq_name = os.path.splitext(os.path.basename(dic['audio_filepath']))[0]
            if uniq_name in out_embeddings:
                out_embeddings[uniq_name] = np.concatenate((out_embeddings[uniq_name], all_embs[i].reshape(1, -1)))
            else:
                out_embeddings[uniq_name] = all_embs[i].reshape(1, -1)
...
def main():
    ...
    parser.add_argument(
        "--manifest", type=str, required=False, help="Path to manifest file",
        default='/home/kpuvvada/diar_test.json'
    )
    parser.add_argument(
        "--model_path",
        type=str,
